...
oauth_token_var = contextvars.ContextVar("oauth_token", default=None)
...
def get_cal_creds(user_id: str, engine: sqlalchemy.ext.asyncio.engine.AsyncEngine):
    ...
    token = oauth_token_var.get()
    if token:
        logging.info("Using OAuth Token extracted from A2A Request Authorization header.")
        return Credentials(token=token)

    SCOPES = ['https://www.googleapis.com/auth/calendar', 'https://www.googleapis.com/auth/calendar.events']
    
    logging.info("Falling back to Application Default Credentials (ADC) for Google Calendar auth.")
    try:
