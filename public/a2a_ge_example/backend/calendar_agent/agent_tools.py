...
engine = None  # Injectable default engine object

logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

async def get_token_from_db(user_id: str, engine: sqlalchemy.ext.asyncio.engine.AsyncEngine) -> dict | None:
    """Retrieves a user's token from the AlloyDB database."""
    async with engine.connect() as conn:
        result = await conn.execute(
            sqlalchemy.text("SELECT access_token, refresh_token, expires_at FROM oauth_tokens WHERE user_id = :user_id"),
            {"user_id": user_id},
        )
        row = result.fetchone()
        if not row:
...
def get_cal_creds(user_id: str, engine: sqlalchemy.ext.asyncio.engine.AsyncEngine):
    import google.auth
    SCOPES = ['https://www.googleapis.com/auth/calendar', 'https://www.googleapis.com/auth/calendar.events']
    
    logging.info("Forcing Application Default Credentials (ADC) for Google Calendar auth.")
    try:
        creds, _ = google.auth.default(scopes=SCOPES)
        return creds
    except Exception as e:
        logging.error(f"Error getting ADC: {e}")
        return None
...
def check_cal(
    ...
    try:
        ...
        if "email" not in tool_context.state:
            get_user_profile(tool_context)
        creds = get_cal_creds(tool_context.state.get("email", "unknown@example.com"), engine)
        if not creds:
            logging.error("google.auth.default() failed to return credentials in get_cal_creds. Aborting check_cal.")
            return []
        ...
        error_msg = f'An HTTP error occurred in check_cal: {error}'
        logging.error(error_msg, exc_info=True)
        return [{"error": error_msg}]
    except Exception as e:
        error_msg = f"An unexpected error occurred in check_cal: {str(e)}"
        logging.error(error_msg, exc_info=True)
        return [{"error": error_msg}]
...
def create_event_with_details(
  ...
  if "email" not in tool_context.state:
      get_user_profile(tool_context)
  creds = get_cal_creds(tool_context.state["email"], engine)
...
def create_calendar_event(
  ...
  try:
      ...
      if start_datetime >= end_datetime:
          err_msg = "Error: Start time must be before end time."
          print(err_msg)
      ...
      err_msg = f'An error occurred: {error}'
      print(err_msg)
      return {"error": err_msg}
  except Exception as e:
      err_msg = f"Error creating event: {str(e)}"
      print(err_msg)
      return {"error": err_msg}
